# Changelog – Computer Compendiums

> Format: YYYY-MM-DD — Short header
> Key changes in bullet points.

## 2026-03-25 — Update
- **Networking** - Chapter 3.5-3.6 expanded. More depth and examples

## 2026-03-24 — Update
- **Networking** - Chapter 3.4 expanded. More depth and examples

## 2026-03-23 — Update
- **Networking** - Chapter 3 started. Chapter 3.1, 3.2 and 3.3 expanded

## 2026-03-22 — Update
- **Networking** - Chapter 2 done. 2.3 expanded

## 2026-03-19 — Update
- **Networking** - Chapter 1-2.1 updated

## 2026-03-16 — Update
- **Computer Technology** Book complete. PDF updated<br>
- **Networking** - Chapter 1 updated

## 2026-03-13 — Update
- **Computer Technology** Updated Licensing Information<br>
- **Networking** - Updated Licensing Information<br>
- **Computer Technology** Added questions and formatted links ch 1-10<br>
- **Computer Technology** - Fixed navigation<br>
- **Networking** - Fixed navigation

## 2026-03-12 — Update

- **Computer Technology** Everything up to chapter 11 is rewritten and checked to add more depth

## 2026-03-10 — Update

- **Computer Technology** updated to chapter 8 witch has ben completely rewritten and now has a more advanced, and in depth approach

## 2026-03-09 — Update

- 2026-1 version of the **Computer Technology** book (Markdown migration complete).<br>
- **Networking**: Updated chapters and initiated spring content update.
- Migration to a new automated Markdown structure for better Git integration.

## 2025-11-17 — New Chapters

- **Networking**: Updated Appendix II with RIP and OSPF.

## 2025-08-19 — Content Expansion

- **Computer Technology**: 9.7 Common attacks against clients and systems.<br>
- **Networking**: 2.5 Common network attacks and defense strategies.

## 2025-08-18 — GitHub Pages Structure
- Moved to **Just the Docs** theme under the `docs/` directory.<br>
- Created base pages: `docs/index.md`, `docs/datorteknik/index.md`, `docs/network/index.md`.<br>
- PDF links prepared in `docs/_config.yml` under `aux_links`.<br>
- Cleanup of erroneously created files and comment residue.

## 2025-08-17 — Computer Technology v1.1
- Multiple headings updated and expanded.
- Rewrote bullet-point chapters into **narrative text** (e.g., USB section).
- **GPU section** expanded with a stronger focus on modern hardware.
- **Appendix**: Approximately **300 terms and definitions** added.

## 2025-08-17 — Networking v1.1
- **New cover design**.
- **License information** updated and clarified.

## 2025-XX-YY — v1.0 (First Public Release)
- Initial public release of both books.
