---
layout: default
title: "7.6 Installation av Ubuntu Server och Webmin"
parent: "Datorteknik"
nav_order: 706
---
...
Ladda ner Ubuntu ServerBesök:https://ubuntu.com/download/serverLadda ner den senaste LTS-versionen (t.ex. Ubuntu Server 24.04 LTS).

Skapa en startbar USB

Använd till exempel:Rufus (Windows) – https://rufus.ieBalena Etcher – https://etcher.balena.ioStartup Disk Creator (på Ubuntu/Linux)

Skriv ISO-filen till ett USB-minne på minst 4 GB.

Starta datorn från USB

Starta om datorn och välj att boota från USB-enheten.Detta görs oftast via F12, F2, ESC eller DEL beroende på tillverkare.

Välj språk, tangentbord och tidszon

Följ installationsguiden och välj rätt inställningar för språk och region.

NätverksinställningarOm du har en DHCP-server (t.ex. via Proxmox eller OPNsense) får servern IP-adress automatiskt. Annars kan du välja att ställa in en statisk IP-adress direkt i installationen.
...
Lagring och partitioneringVälj hur disken ska användas:Use an entire disk – Ubuntu partitionerar automatisktCustom storage layout – För dig som vill lägga upp egna partitioner (avancerat)

Installera valfria tjänsterUbuntu Server erbjuder färdiga tjänstegrupper att installera direkt:OpenSSHSambaLAMP / LEMPDockerPrint server… och fler
...
Skapa ny virtuell maskinTyp: LinuxVersion: Ubuntu (64-bit)RAM: minst 1–2 GB (helst 4+)Disk: minst 8 GB (helst 20+ GB)
...
Viktigt!: Webmin använder HTTPS med ett självsignerat certifikat, så din webbläsare kommer att ge en varning – det är normalt.Logga in med samma användarnamn och lösenord du skapade vid installationen av Ubuntu Server.
...
Ubuntu Server Download (Ladda ner den senaste versionen av Ubuntu Server ISO) – https://ubuntu.com/download/server

Ubuntu Server Guide (Officiell teknisk dokumentation för Ubuntu Server) – https://ubuntu.com/server/docs

Webmin (Officiell webbplats för administrationsverktyget Webmin) – https://www.webmin.com

Webmin GitHub Repository (Projektets källkod och utvecklingshistorik) – https://github.com/webmin/webmin

Install Webmin on Ubuntu (Detaljerad installationsguide från ComputingForgeeks) – https://computingforgeeks.com/how-to-install-webmin-on-ubuntu/

Webmin Documentation Wiki (Omfattande manual för alla inbyggda moduler i Webmin) – https://doxfer.webmin.com/Webmin/Main_Page

DigitalOcean (Initial Server Setup med Ubuntu – viktiga säkerhetssteg efter installation) – https://www.digitalocean.com/community/tutorials/initial-server-setup-with-ubuntu-22-04

Linuxize (Hur man installerar och konfigurerar SSH på Ubuntu Server) – https://linuxize.com/post/how-to-enable-ssh-on-ubuntu-22-04/

Ubuntu Community (Guide till UFW - Uncomplicated Firewall för att säkra din server) – https://help.ubuntu.com/community/UFW
...
---
**Nästa kapitel:** [7.7 Linux grundläggande kommandon](07.07_Linux_grundlaggande_kommandon.md)