...
Alla moderna Windows-versioner (inkl. Server) måste använda NTFS på systemdisken.
...
Windows-registret är en hierarkisk databas som lagrar konfiguration för operativsystemet, drivrutiner, tjänster och många applikationer. Strukturen består av nycklar (som mappar) och värden (som filer) med olika datatyper, till exempel REG_SZ (text) eller REG_DWORD (heltal). Rotnivån utgörs av så kallade hives:

HKEY_LOCAL_MACHINE (HKLM) – maskinvida inställningar för OS, drivrutiner och tjänster

HKEY_CURRENT_USER (HKCU) – inställningar för den inloggade användaren

HKEY_USERS (HKU) – alla användarprofiler (används bl.a. för att nå andra användares HKCU)

HKEY_CLASSES_ROOT (HKCR) – filtypsassociationer och COM-registreringar

HKEY_CURRENT_CONFIG (HKCC) – en ögonblicksbild av aktuell hårdvaruprofil och vissa drivrutinsinställningar.
...
Det består av nycklar och värden – ungefär som mappar och filer.

Det är centralt för konfiguration av Windows, drivrutiner och applikationer.

Det kan redigeras med Registereditorn (regedit) – kraftfullt men riskabelt.
...
Vad är enhetsbokstäver (t.ex. C: eller D:) i Windows och vad används de till rent praktiskt för användaren?

Vilka är de främsta fördelarna med att använda NTFS jämfört med det äldre filsystemet FAT32?

Beskriv steg för steg hur du ändrar rättigheter för en fil eller mapp via Windows grafiska gränssnitt.

Vad är Windows-registret och vilken typ av information lagras i denna centrala databas?

Varför är det kritiskt att vara extremt försiktig när man gör manuella ändringar i registret?

Vad innebär begreppet "arv" (Inheritance) när det kommer till behörigheter för mappar och undermappar?

Vad är skillnaden mellan NTFS-behörigheter och utdelningsbehörigheter (Share permissions) när man delar ut en mapp på ett nätverk?

Vad är ReFS (Resilient File System) och i vilka sammanhang är det mer lämpligt att använda än NTFS?

Vilket verktyg (program) använder man för att redigera registret manuellt, och vad heter den körbara filen för detta program?

Var på hårddisken lagras registrets filer (så kallade hives) rent fysiskt i Windows-installationen?

### Fördjupningslänkar

Microsoft (File Systems in Windows) – https://learn.microsoft.com/en-us/windows/win32/fileio/file-systems

Microsoft (NTFS Technical Reference) – https://learn.microsoft.com/en-us/windows/win32/fileio/ntfs-technical-reference
...
Microsoft (Introduction to the Windows Registry) – https://support.microsoft.com/en-us/windows/what-is-the-windows-registry-776707f4-66f0-7f80-0285-3c6441d04d46

Wikipedia (Windows Registry Overview) – https://en.wikipedia.org/wiki/Windows_Registry

Microsoft (Resilient File System - ReFS Overview) – https://learn.microsoft.com/en-us/windows-server/storage/refs/refs-overview

Microsoft (How Security Descriptors and Permissions Work) – https://learn.microsoft.com/en-us/windows/win32/secauthz/how-it-works--aces-and-acls

BleepingComputer (Guide to Windows Registry Editor for Beginners) – https://www.bleepingcomputer.com/tutorials/how-to-use-the-windows-registry-editor/

Lifewire (A Complete Guide to NTFS Permissions) – https://www.lifewire.com/ntfs-permissions-explained-4053913

Wikipedia (New Technology File System - NTFS) – https://en.wikipedia.org/wiki/NTFS
...
---
**Nästa kapitel:** [6.3 Fördelar och nackdelar med Windows som operativsystem](06.03_Fordelar_och_nackdelar_med_Windows_som_operativsystem.md)