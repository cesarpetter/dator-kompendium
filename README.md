👉 [README in English](#english)

# Dator-kompendier

Kursmaterial för IT-elever på gymnasiet – omarbetat och optimerat för Git.
*Skrivet av Robin Bräck.*

## 📘 Datorteknik
Hela boken är nu konverterad till Markdown för optimal läsbarhet online.
* [**Öppna boken (Innehållsförteckning)**](docs/datorteknik/index.md)

## 🌐 Nätverksteknik
Nu uppdaterad och migrerad till samma smidiga kapitelformat som datortekniken.
* [**Öppna boken (Innehållsförteckning)**](docs/network/index.md)

---

## ⤓ Ladda ner som PDF
- [Datorteknik (PDF)](docs/assets/pdf/datorteknik.pdf)
- [Nätverksteknik (PDF)](docs/assets/pdf/network.pdf)

## Nytt & på gång
- 📝 **Changelog:** [View changelog](docs/changelog.md)
- ✅ **TODO:** [View progress](docs/todo.md)

## Om materialet
Syftet är att ge ett teoretiskt komplement till praktiska labbar. Materialet bygger på mina år som IT-lärare och är skrivet för att vara lättillgängligt för gymnasieelever. Varje kapitel innehåller:
1. **Flödande text** baserad på senaste kursplanerna.
2. **Kontrollfrågor** för repetition.
3. **Fördjupningslänkar** för de som vill veta mer.

---

## Feedback
Jag tar tacksamt emot feedback: **robin.m.brack@gmail.com**

## Supporta projektet ☕
Om du vill stödja arbetet kan du skicka en slant via Swish.
![Swish](swish.png)

---

<a name="english"></a>
# Computer Compendiums (English)

Course material for upper-secondary IT students — written by Robin Bräck.

**Read online**
- 📘 **Computer Technology (web):** [Table of contents](docs/datorteknik/index.md)
- 🌐 **Networking (web):** [Table of contents](docs/network/index.md)

**Download as PDF**
- ⤓ **Computer Technology (PDF):** [Download](docs/assets/pdf/datorteknik.pdf)
- ⤓ **Networking (PDF):** [Download](docs/assets/pdf/network.pdf)

---

## What’s new & what’s next
- 📝 **Changelog:** [View changelog](docs/changelog.md)
- ✅ **TODO:** [View progress](docs/todo.md)

## About the material
This project is a theoretical companion to hands-on labs and exercises. The content consists of refined notes from over seven years of teaching IT, optimized for modern digital learning environments.

[↑ Back to Swedish](#dator-kompendier)

## Feedback
I’d love to hear your thoughts: **robin.m.brack@gmail.com**

## Support the project ☕
If you want to support the work, you can send a tip via Swish by scanning the QR code below.  
![Swish QR code](swish.png)

## License
This material is licensed under **Creative Commons BY-NC-SA 4.0**.  
You may share and adapt the material, but you must give attribution, you may not use it commercially, and you must distribute derivatives under the same license.
